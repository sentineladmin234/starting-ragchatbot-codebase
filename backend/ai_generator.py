...
"""
    ...
    def _execute_tool_round(self, messages: List[Dict],
                           system_content: str,
                           tools: Optional[List],
                           tool_manager,
                           round_number: int,
    ...
    def _should_continue_rounds(self, response, round_number: int, max_rounds: int) -> bool:
        """
    ...
    def _build_round_aware_system_prompt(self, base_system: str, 
                                       round_number: int, 
                                       max_rounds: int) -> str:
        """
        ...
        if round_number == 1:
            round_context += "This is your first opportunity to use tools for this query.\n"
            if max_rounds > 1:
                round_context += f"You can make additional tool calls in Round {round_number + 1} if needed.\n"
        elif round_number == max_rounds:
            round_context += "This is your final round - make any remaining tool calls needed.\n"
            round_context += "After this round, provide your complete final answer.\n"
        else:
            round_context += f"You can make additional tool calls in the next round if needed.\n"
