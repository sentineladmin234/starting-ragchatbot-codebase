...
class TestVectorStore(unittest.TestCase):
    ...
    def test_search_with_max_results_zero_bug(self):
        ...
        self.assertTrue(
            results.is_empty(), "Search should return empty results when max_results=0"
        )
    ...
    def test_search_with_fixed_max_results(self):
        ...
        if not results.error:
            self.assertFalse(
                results.is_empty(), "Search with explicit limit should return results"
            )
            print(
    ...
    def test_get_all_courses_metadata(self):
        ...
        lessons = [
            ...
            Lesson(
                lesson_number=2, title="Advanced", lesson_link="https://example.com/2"
            ),
        ]
        course = Course(
            title="Test Course",
...
class TestRealVectorStoreData(unittest.TestCase):
    ...
    def test_real_data_loading(self):
        ...
        if course_count > 0:
            ...
            print(
                f"   Search with explicit limit=3: {len(results_with_limit.documents)} documents"
            )
