...
class TestConfig(unittest.TestCase):
    ...
    def test_chunk_size_valid(self):
        ...
        self.assertLess(
            config.CHUNK_SIZE, 5000, "CHUNK_SIZE too large - may exceed token limits"
        )

    def test_chunk_overlap_valid(self):
        """Test that chunk overlap is reasonable"""
        self.assertGreaterEqual(
            config.CHUNK_OVERLAP, 0, "CHUNK_OVERLAP cannot be negative"
        )
        self.assertLess(
    ...
    def test_max_history_valid(self):
        ...
        self.assertGreaterEqual(
            config.MAX_HISTORY, 1, "MAX_HISTORY should be at least 1 for context"
        )
    ...
    def test_chroma_path_valid(self):
        ...
        self.assertNotEqual(
            config.CHROMA_PATH.strip(), "", "CHROMA_PATH is empty or whitespace"
        )
    ...
    def print_config_status(self):
        ...
        print(
            f"MAX_RESULTS: {config.MAX_RESULTS} {'❌ CRITICAL ISSUE' if config.MAX_RESULTS == 0 else '✅'}"
        )
        print(
